...
export const UserTable: React.FC<Props> = ({ users, onEdit, onToggleStatus, onDelete }) => {
  ...
  const [roleFilter, setRoleFilter] = useState<'all'|'supervisor'|'employee'>('all');
  const [statusFilter, setStatusFilter] = useState<'all'|'active'|'suspended'>('all');
  const [confirm, setConfirm] = useState<ConfirmState>(null);

  const supervisorMap = Object.fromEntries(users.filter(u=>u.role==='supervisor').map(u=>[u.id,u.name]));
  ...
  return (
    <>
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden dark:border-slate-700 dark:bg-slate-800">
        {/* Toolbar */}
        <div className="flex flex-wrap items-center gap-3 border-b border-slate-100 px-5 py-3.5 dark:border-slate-700">
          ...
          <span className="ms-auto text-xs text-slate-400 dark:text-slate-500">{filtered.length} {filtered.length===1 ? 'result' : 'results'}</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[700px] text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {filtered.length===0 ? (
                <tr><td colSpan={6} className="px-5 py-10 text-center text-sm text-slate-400 dark:text-slate-500">{t('noUsersFound')}</td></tr>
              ) : filtered.map(user => (
                <tr key={user.id} className="group hover:bg-slate-50 transition-colors dark:hover:bg-slate-700/40">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white ${avatarColor(user.id)} ${user.status==='suspended'?'opacity-50':''}`}>
                        {initials(user.name)}
                      </div>
                      <div>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex flex-col gap-0.5">
                      ...
                      {user.role === 'supervisor' && user.mainJob && <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate max-w-[180px]" title={user.mainJob}>{user.mainJob}</p>}
                    </div>
                  </td>
                  <td className="px-5 py-3.5 text-sm text-slate-600 dark:text-slate-400">
                    {user.supervisorId && supervisorMap[user.supervisorId] ? supervisorMap[user.supervisorId] : <span className="text-slate-300 dark:text-slate-600">—</span>}
                  </td>
                  <td className="px-5 py-3.5">
